...
const ChatFileUpload:FC<ChatFileUploadProps> = ({userData, workspaceData, channel, toggleFileUploadModal, recipientId}) => {
    ...
    async function handleUpload(values: z.infer<typeof formSchema>){
        ...
        const fileName = `chat/${fileTypePrefix}-${uniquedId}`;
        ...
        if(recipientId){
            const {data: directMessageData, error: dmError} = await supabase.from("direct_messages").insert({
                file_url: data.path,
                user_id: userData.id,
                ...
                <div className="space-y-2 text-sm">
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(handleUpload)} className="space-y-8">
                            <FormField control={form.control} name="file" render={({field}) => <FormItem>
