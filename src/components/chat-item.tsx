...
import { MdOutlineAdminPanelSettings, MdOutlineAssessment, MdOutlineAssistantPhoto } from "react-icons/md";
...
const ChatItem:FC<ChatItemProps> = ({content, currentUser, id, user, timestamp, fileUrl, deleted, isUpdated, socketUrl, socketQuery, channelData}) => {
    ...
    const DeleteDialog = () => <Dialog onOpenChange={() => setOpenDeleteDialog(!openDeleteDialog)} open={openDeleteDialog}>
      <DialogTrigger>
        <Trash size={20} />
      </DialogTrigger>
      <DialogContent>
          ...
          <div className="text-center">
            {isPdf && (
              <div className="items-start justify-center gap-3 relative">
                ...
                <Link href={publicUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
                  View PDF
                </Link>
              </div>
            )}
        ...
        <div className="flex flex-col gap-2">
          <Button onClick={() => setOpenDeleteDialog(false)} className="w-full" variant='secondary' disabled={isDeleting}>
            No, Cancel
          </Button>
          <Button onClick={handleDelete} className="w-full" variant="destructive" disabled={isDeleting}>
            Yes, Delete
          </Button>
        </div>
      </DialogContent>
    </Dialog>;
    const EditableContent = () => (isEditing ? <Form {...form}>
    ...
    </Form> : <div className={cn('text-sm', {'text-xs opacity-90 italic':deleted})} dangerouslySetInnerHTML={{__html: content ?? ''}}></div>)
    return <div className="relative group flex items-center hover:bg-black/5 px-1 py-2 rounded transition w-full">
        <div className="flex gap-x-2">
            <div className="cursor-pointer hover:drop-shadow-md transition">
                <Avatar>
                    <AvatarImage src={user.avatar_url} alt={user.name ?? user.email} className="object-cover w-full h-full" />
                    <AvatarFallback className="bg-neutral-700">
                        <Typography variant="h4" text={user.name?.slice(0,2) ?? 'UN'} />
                    </AvatarFallback>
                </Avatar>
            </div>
