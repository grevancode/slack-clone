...
const SidebarNav: FC<SidebarNavProps> = ({userWorkspacesData, currentWorkspaceData}) => {
                            ...
                            <PopoverContent className="p-0" side="bottom">
                                <Card className="w-[350px] border-0">
                                    <CardContent className="flex p-0 flex-col">
                                        {switchingWorkspace ? (<div className="m-2"><ProgressBar /></div>) : (userWorkspacesData.map(workspace => {
                                            const isActive = workspace.id === currentWorkspaceData.id;
                                            return(
                                                <div key={workspace.id} className={cn(isActive && `${backgroundColor} text-white`  ,"cursor-pointer px-2 py-1 flex gap-2")} onClick={() => {() => !isActive && switchWorkspace(workspace.id)}}>
                                                <Avatar>    
                                                ...
                                                <div>
                                                    <Typography variant="h3" text={workspace.name} className="text-sm" />
